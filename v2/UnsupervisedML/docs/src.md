...
1. plot_anomalies(df, outliers) : plots the scatter plot for the outliers based on the model used and for the comparison on the side it also plot another scatter plot in the raw form without outliers marked out. (used implicitly only within the file never got called off directly in any jupyter notebook file)

2. find_threshold(df) : plots the curve for the data to detect the sharp drop, based on which we deicde the threshold value which when combined with the socres help find out the contamination for the Isolation forest

3. find_contamination(df, scores, threshold) : gives the exact value of the contamination based on the data, scores and threshold (from the previous function) given to it

4. outliers_isolation_forest(df, contamination) : plot the outliers using the Isolation forest based on the data and value of the contamination provided

- (the functions no. 2, 3 and 4 works together for the same dataset only, preventing the leakage but the written format is general).

5. outliers_lof(df, n) : plot the outliers using the Local outlier factor (LOF) based on the value of n (which is n_neighbors) provided. (the rest of the paramters like leaf_size and algorithm as assumed to be default i.e. algorithm = 'auto', leaf_size = 30 while creating the object)

6. outliers_dbscan(df, eps) : plot the outliers using the DBScan clustering based on the given eps value for the given dataset.